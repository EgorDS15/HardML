...
class Solution:
    def __init__(self, glue_qqp_dir: str, glove_vectors_path: str,
        ...
        """
            min_token_occurancies — минимальное количество раз, которое слово (токен) должно появиться в выборке,
            чтобы не быть отброшенным как низкочастотное. При значении, равном единице, остаются все слова, которые
            представлены в датасете.

            emb_rand_uni_bound — половина ширины интервала, из которого равномерно (uniform) генерируются вектора
            эмбеддингов (если вектор не представлен в наборе GloVe). Если параметр равен 0.2, то каждая компонента
            вектора принадлежит U(−0.2,0.2)

            freeze_knrm_embeddings — флаг, указывающий на необходимость дообучения эмбеддингов, будут ли по ним считаться
            градиенты (при True дообучение происходить не будет).
    ...
    def hadle_punctuation(self, inp_str: str) -> str:
        for i in string.punctuation:
            inp_str = inp_str.replace(i, '').strip()
        return inp_str
    ...
    def _filter_rare_words(self, vocab: Dict[str, int], min_occurancies: int) -> Dict[str, int]:
        if min_occurancies < 1:
            raise ValueError("Parameter min_occurancies should be int type and has a value 1 or bigger")
    ...
    def get_all_tokens(self, list_of_df: List[pd.DataFrame], min_occurancies: int) -> List[str]:
        ...
        for df in list_of_df:
            for colmn in df:
                for row in df[colmn]:
                    uniq_text.add(str(row))

        # разбиение на токены, обрезание пробела и приведение к нижнему регистру
        prep_text = []
        for i in uniq_text:
            prep_text.append([i for i in self.simple_preproc(i)])

        # extracted tokens
        all_tokens = [item for sublist in prep_text for item in sublist]
        ...
        result = [key for key in self._filter_rare_words(freq_table, min_occurancies).keys()]
