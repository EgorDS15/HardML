...
class GaussianKernel(torch.nn.Module):
    """
    Основа архитектуры - одно гаусово ядро, которое будет сварачивать нашу матрицу матчинга и будет получать
    активации, поверх которых будем строить лиенейный классификатор
    """
    ...
    def forward(self, x):
        # смотрим как текущий признак отличается от центра ядра: x - self.mu. Чем дальше значение от ядра, тем меньше
        # значение на выходе. Ширина "купола": self.sigma ** 2
...
class KNRM(torch.nn.Module):
    def __init__(self,
        ...
        """
        :param embedding_matrix: матрица эмбеддингов N*D. N - размер словаря эмбединга, D - размер вектора в этой матрице
        :param freeze_embeddings: хотим ли дообучать эмбединги
        :param kernel_num: количество ядер
        :param sigma: параметр ядра
        :param exact_sigma:
        :param out_layers: указывает на структуру финального линейного слоя
        """
        super().__init__()
        # перевод предобученных numpy эмбедингов в torch эмбединги
    ...
    def _get_kernels_layers(self) -> torch.nn.ModuleList:
        # Создание ядер. Используя Гаусово ядро создаем список ядер - torch.nn.ModuleList().
        # Но нужно понять по какой формуле расчитывается значение mu - центр ядра, на которое
        # обращается внимание. Как находим формулу - числа от -1 до 1. рсаполагаем числа в интервал от 0 до 1,
        # удваиваем и отнимаем 1, тем самым сдвигаем влево шкалу.
